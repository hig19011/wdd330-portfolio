...
export default class Quake {
  ...
  async getEarthQuakesByRadius(position, startOn, endOn, radius = 500) {
    // use the getJSON function and the position provided to build out the correct URL to get the data we need.  Store it into this._quakes, then return it
    let url = this.baseUrl + `&starttime=${startOn}&endtime=${endOn}&latitude=${position.latitude}&longitude=${position.longitude}&maxradiuskm=${radius}`;
